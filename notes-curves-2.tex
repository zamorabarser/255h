...
\begin{definition}
    ...
    \[  \int_C F \cdot d\gamma   : = \int_C (P \, dx + Q\,  dy + R \, dz) : = \int_a^b F(\gamma (t) ) \cdot \gamma '(t) \, dt  .                \]
\end{definition}
...
\begin{example}
    ...
    \end{center}
    Since $F$ is pointing right, it is helping $\alpha $ perform its trajectory, it is not helping nor preventing $\beta$ from performing its trajectory, and is pushing $\gamma$ against its trajectory. Let $A\subset \mathbb{R}^2$ be the image of $\alpha$, $B\subset \mathbb{R}^2$ be the image of $\beta$, $C\subset \mathbb{R}^2$ be the image of $\gamma$.  From here we intuitively deduce that 
    \begin{center}
        $\int_{A} F \cdot d\alpha $ is positive

        
        $\int_{B} F \cdot d\beta $ is zero

        
        $\int_{C} F \cdot d\gamma $ is negative
    \end{center}
    ...
    Basically:
    \begin{itemize}
        \item if $\alpha$ goes with the flow of $F$, the integral $\int_{A} F \cdot d\alpha $ is positive.
        \item if $\gamma $ is swimming against the current, the integral $\int_{C} F \cdot d\gamma $ is negative.
    \end{itemize}

\end{example}

The integral $\int_C F \cdot d\gamma $ is also called the work of $F$ along the trajectory $\gamma$. 
...
Then
\begin{align*}
\int_C F \cdot d\gamma    & = \int_0 ^2  \langle -4t  , t^6 + 3t^2 - 9 , t^2 - 4 \rangle   \cdot \langle 2t , 2, 3t^2 \rangle \, dt \\
...
Then
\begin{align*}
\int_C F \cdot d\gamma    & = \int_0 ^3  \langle  t + 1 , e^t , - t^2   \rangle   \cdot \langle  e^t , - 2t , 1  \rangle \, dt \\
...
\begin{theorem}
    ...
    In particular, the integral $\int_C \nabla f \cdot d\gamma $ does only depend on the endpoints of $C$ and not on the trajectory. 
\end{theorem}
...
Therefore, 
\begin{align*}
    \int_C F \cdot d\gamma  & = f(   \pi ^2 \sqrt{\pi ^2 + 1 } , e ^{\pi ^2} , - \pi ^2    ) - f (0,1,0) \\
...
Therefore, 
\begin{align*}
     \int_{C} F \cdot d\gamma  & = \int_0 ^{2 \pi } F(\gamma (t)) \cdot \gamma ' (t)  \, dt  \\ 
...
\begin{align*}
   \int_C F \cdot d\gamma  & = f(\gamma (2 \pi )) - f (\gamma (0)) \\
    &= f (1,0) - f (1,0) \\
    & = 0                            
\end{align*}
