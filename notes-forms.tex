...
\begin{proposition}
    Note that when you exchange the order of two entries under $\wedge$, you get a minus sign. For example
...
\begin{example}[$k=0$, $n=3$] A differentiable $0$-form in $\mathbb{R}^3$ has the formula
\[          \omega  = f(x ,y, z)     \]
 A $0$-manifold is a point $p \in \mathbb{R}^3$. Then the integral of $\omega $ over the $0$-manifold $p$ is 
 \[            f(p)                  \]
\end{example}
